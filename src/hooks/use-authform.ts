...
export const useAuthForms = () => {
  ...
  const validateLoginData = (data: LoginData): AuthError | null => {
    if (!data.email || !data.password) {
      return { message: 'Veuillez remplir tous les champs', code: 'EMPTY_FIELDS' };
    }
    if (!data.email.includes('@')) {
      return { message: 'Email invalide', code: 'INVALID_EMAIL' };
    }
    if (data.password.length < 8) {
      return { message: 'Le mot de passe doit contenir au moins 8 caractères', code: 'INVALID_PASSWORD' };
    }
    return null;
  };
  ...
  const validateRegisterData = (data: RegisterData): AuthError | null => {
    ...
    if (!data.username) {
      return { message: "Le nom d'utilisateur est requis", code: 'EMPTY_USERNAME' };
    }
    if (data.password !== data.confirmPassword) {
      return { message: 'Les mots de passe ne correspondent pas', code: 'PASSWORD_MISMATCH' };
    }
    return null;
  };
  ...
  const handleLogin = async (e: React.FormEvent): Promise<void> => {
    ...
    try {
      ...
      setError({ message: 'Erreur lors de la connexion', code: 'LOGIN_ERROR' });
    }
  };
  ...
  const handleRegister = async (e: React.FormEvent): Promise<void> => {
    ...
    try {
      ...
      setError({ message: "Erreur lors de l'inscription", code: 'REGISTER_ERROR' });
    }
  };
